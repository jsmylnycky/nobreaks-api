  ...
  return new Promise((resolve, reject) => {
    ...
    getRaids(guildId)
      .then((raidData) => {
        ...
        if (raidDate !== LATEST) {
          raidIndex = _.findIndex(raids, {raidDate: moment(raidDate, 'YYYY-MM-DD').format('MM-DD-YYYY')});
      ...
      .then((items) => {
        ...
        resolve({
          nextRaidDate: (raidIndex === 0) ? null : items.nextRaidDate = moment(raids[raidIndex - 1].raidDate, 'MM-DD-YYYY').format('YYYY-MM-DD'),
          prevRaidDate: (raidIndex === raids.length - 1) ? null : items.nextRaidDate = moment(raids[raidIndex + 1].raidDate, 'MM-DD-YYYY').format('YYYY-MM-DD'),
          raidDate: moment(raids[raidIndex].raidDate, 'MM-DD-YYYY').format('YYYY-MM-DD'),
          items: items
        });
      }, (err) => {
        reject({error: {message: err}});
...
function getMembersEPGP(guildId, ranks) {
  return new Promise((resolve, reject) => {
    characterService
