...
function getMembersEPGP(guildId, options = {}) {
  return new Promise((resolve, reject) => {
    characterService
      ...
      .then((character) => {
        ...
        if (options.ranks) {
          let parsed = options.ranks.split(',')
        ...
        if (options.remove) {
          let parsed = options.remove.split(',')

          populate.match = Object.assign({}, populate.match, {
            name: { $nin: parsed }
          })
        }
  ...
  return new Promise((resolve, reject) => {
    EPGPRaid
      ...
      .then((raidData) => {
         resolve(
           _.sortBy(raidData, [(o) => {
             return moment(o.raidDate, 'MM-DD-YYYY').format('YYYY-MM-DD');
           }])
           .reverse()
         );
      }, (error) => {
